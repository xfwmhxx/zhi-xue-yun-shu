import ARTCAICallEngine, {
  ...
  type AICallEngineConfig,
  AICallErrorCode,
  AICallSendTextToAgentRequest,
  AICallState,
  type AICallSubtitleData,
  type AICallVisionCustomCaptureRequest,
...
export default abstract class AUIAICallController extends EventEmitter<AUIAICallControllerEvents> {
  ...
  protected addEngineListener() {
    ...
    this._currentEngine.on('errorOccurred', (errorCode: number, msg: string) => {
      ...
      logger.error(
        'AICallErrorOccurred',
        new AICallAgentError(msg || `code: ${errorCode}`, errorCode),
      )
    })
  ...
  protected async describeAIAgentInstance(instanceId: string) {
    ...
    try {
      const agentConfig = await standard.describeAIAgentInstance(
        this.userId,
        this.token,
        this.config.region,
        instanceId,
  ...
  setAgentView(view: HTMLVideoElement | string): void {
    logger.info('Controller', 'SetAgentView', {
      view: typeof view === 'string' ? `#${view}` : view.nodeType,
    })
